...
This is because using chat as the primary form factor limits what can be done with the underlying technology. 

The blank-page, prompt-based UI has become ubiquitous, but AI leaders, technology thinkers and UX experts increasingly acknowledge the constraints. 
...
3. **Context/task-specific chat**: deployed within software system that is itself the context for a specific task (Cursor) or within a task-specific feature in a larger software system (chatting with AI about a specific account in Gong).
...
This problem is much larger on the general end of the spectrum. The more task-specific the context, the more pre-prompting can be built into the system to remove the burden from the end users. 
...
This makes basic sense: if a user can do anything with an open-ended tool, they have to exert a significant amount of creative cognitive effort to decide what they want to use it for. Most users will default to what’s simple and familiar, which I suspect is a major reason behind the surprisingly low daily and weekly active user numbers that Benedict Evans highlighted[^2]. 

Web and app interfaces give designers more opportunities to assist users facing a blank input field and the major apps are already incorporating one-click, pre-scripted prompts as starting points for new users (and more personalized prompts for existing, logged-in users). Those GUI-based UX details will help educate users in a way that voice never could, which will likely accelerate adoption of more advanced use cases, but I doubt they will be enough to turn a significant percentage of casual users into power users. 
...
_At ~800 million ChatGPT users[^8] and counting, combined with a roaring API business, perhaps these aren’t problems for the likes of OpenAI._
...
AI is expensive to run and many companies are selling products at a loss—even those with record-breaking growth. People who have studied this issue on a much deeper level have discovered that “every _single_ company offering any kind of generative AI service...is, from every report [the author] can find, losing money...”[^10] Indeed, the cost of users saying “thank you” at the end of a chat is astounding[^11]. 
...
Conversations can introduce new ideas and help you think more clearly, so all kinds of iterative processes can benefit from chat. Writing code (either in an IDE or a platform like Vercel’s v0), creating content, research and other functions all work really well, especially when the context of the entire exchange is used to make subsequent responses more helpful. 

## What does the future of AI interfaces look like? 

As costs decrease and we continue to learn about adoption, we will move beyond today's chat-based interfaces and see a massive wave of new user experiences built on AI. 

This next wave will include agents that decrease the need for interfaces, proactive outputs (with disappearing AI) and multi-modal UIs. 

### Fewer interfaces due to agents

One interesting development has been using chat as an interface to completely separate software systems through MCP servers. I can connect Linear, Notion, GitHub and other services to Claude and perform all kinds of actions in those services. Agents can perform actions _across_ systems. It’s an incredibly powerful paradigm that is in the early stages of changing how knowledge work is performed. 

One result of the MCP architecture is that we will use fewer interfaces in the future and/or spend much more time in the ‘central’ interfaces that can work with other applications. The current manifestations come in two flavors: 

**Controlling one system through another system’s interface** 
...
**Dedicated centralized interfaces**
...
### Dynamically generated interfaces...maybe
...
As costs fall and it becomes economic to experiment with more proactive experiences, a significant shift will be delivering output to users automatically, as opposed to waiting for input. Considering all of the context that LLMs can ingest, this is going to be a fascinating and powerful development. 

**OpenAI’s release of Pulse**[^17] in ChatGPT last week is a significant step beyond user-initiated chat towards a future where AI is proactively delivering output to users. I tried Pulse on a work account that I primarily use for product and market research, so the results were underwhelming, but there’s significant potential, especially if you use ChatGPT for a wide variety of use cases. 
...
I’m very interested to see if a single interface paradigm (beyond chat) emerges in the future. My hypothesis is that patterns will develop as we determine what works best, but the common thread will be multi-modal interfaces that pack a huge amount of complexity into workflows that feel far simpler, using the best form factor every step of the way. 
...
[^2]: Evans’ [post on AI adoption](https://www.ben-evans.com/benedictevans/2025/5/25/genais-adoption-puzzle?utm_source=ericdoddsdotcom) is fascinating. Only 5-15% of people use AI everyday and most use it once a week. 
...
[^6]: You can read more about literacy statistics and find links to the original reports in a post from the [American Public Media Research Lab](https://www.apmresearchlab.org/10x-adult-literacy). 

[^7]: Despite advanced capabilities, most people use voice assistants to perform a small number of very basic tasks. You can read more on the [Nielsen Norman Group blog](https://www.nngroup.com/articles/intelligent-assistants-poor-usability-high-adoption/). 

[^8]: In a [recent study](https://openai.com/index/how-people-are-using-chatgpt/), OpenAI published that ChatGPT has over 700 million active users. 
...
[^10]: You can read about the mind-blowing cost of AI and how much money companies are losing on [Where’s Your Ed](https://www.wheresyoured.at/why-everybody-is-losing-money-on-ai/).

[^11]: Sam Altman [confirmed on X](https://x.com/sama/status/1912646035979239430) that when users say “thank you,” it costs tens of millions of dollars. 

[^12]: [OpenAI’s report](https://openai.com/index/how-people-are-using-chatgpt/) on how people use ChatGPT confirms that search, what they call “asking” tasks, is the primary use cases. 

[^13]: You can read about Linear’s Cursor background agents [on their changelog](https://linear.app/changelog/2025-08-21-cursor-agent). 

[^14]: You can read about Notion 3.0 [on their blog](https://www.notion.com/blog/introducing-notion-3-0). 

[^15]: You can read about Raycast AI [on their website](https://www.raycast.com/core-features/ai).
...
[^19]: You can read about Tab, Cursor’s model for autocomplete, [in their docs](https://cursor.com/docs/tab/overview). 
